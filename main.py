...
class Maze:
    def __init__(self, X, Y, pos_start = (1, 1), pos_end = None):
        ...
        if pos_end is not None: self.pos_end =  min(pos_end[0], X), min(pos_end[1], Y)  
        else: self.pos_end = (X - 2, Y - 2) 
    ...
    def FindingExitRoad(self, pathway) -> None:
            self.exit_Road = pathway.copy()

    
    def printExitRoad(self) -> None:
        maze_show: list[list[str]] = func.copyMaze(self.maze)
        len_exit_road: int = len(self.exit_Road)
        for pos in range(1, len_exit_road):
    ...
    def printMaze(self, maze = None) -> None:
        ...
        os.system('cls')
        for arr in maze: print(''.join(arr))
        time.sleep(0.05)
